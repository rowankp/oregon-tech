 ...
 */
static bool update_max_ptr(uint8_t *new_value)
{
...
static bool parent_is_split(size_t index)
{
    index = (index - 1) / 2;
    return (bool) ((node_is_split[index / 8] >> (index % 8)) & 1);
}
...
{
    if (!request || request + HEADER_SIZE > MAX_ALLOC)
        return NULL;
...
void free(void *ptr)
{
    ...
    size_t bucket = bucket_for_request(*(size_t *) ptr + HEADER_SIZE);
    size_t i = node_for_ptr((uint8_t *) ptr, bucket);
